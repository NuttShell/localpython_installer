...
function Invoke-PythonWithOutput {
    ...
    $psi.EnvironmentVariables["PYTHONHOME"]       = [System.IO.Path]::GetDirectoryName($PythonExe)
    $psi.EnvironmentVariables["PYTHONPATH"]       = [System.IO.Path]::GetDirectoryName($PythonExe)

    $proc           = [System.Diagnostics.Process]::new()
    $proc.StartInfo = $psi
    $proc.Start() | Out-Null

    $sigPat  = "Collecting |Downloading |Installing |Successfully installed|[Ee]rror|ERRO|Traceback|ImportError|ModuleNotFound"
    $lastErr = ""
    ...
    $script:_lastProcErr = $lastErr

    $proc.WaitForExit()
    return $proc.ExitCode
}

function Get-LastProcError { return $script:_lastProcErr }

function Get-PipWhlUrl {
    try {
        $json = Invoke-WebRequest -Uri "https://pypi.org/pypi/pip/json" -UseBasicParsing -TimeoutSec 15
        $data = $json.Content | ConvertFrom-Json
        $whl  = $data.urls | Where-Object { $_.filename -match "^pip-.*-py3-none-any\.whl$" } |
                Select-Object -First 1
        return $whl.url
    } catch { return "" }
}

function Install-PipFromWhl {
    param([string]$PythonDir, [string]$DownloadDir)
    Update-GUI -Status "Fetching pip wheel URL..." -Progress 83 -Detail "pypi.org/pypi/pip/json"
    $whlUrl = Get-PipWhlUrl
    if (-not $whlUrl) {
        Show-Error "Failed to get pip wheel URL from PyPI"
        return $false
    }
    $whlFile = Join-Path $DownloadDir "pip.whl"
    $ok = Download-File -Url $whlUrl -DestinationPath $whlFile `
                        -Description "pip wheel" -ProgressFrom 83 -ProgressTo 86
    if (-not $ok) {
    ...
    Update-GUI -Status "Installing pip from wheel..." -Progress 87 -Detail $whlFile
    Add-Type -AssemblyName System.IO.Compression.FileSystem
    [System.IO.Compression.ZipFile]::ExtractToDirectory($whlFile, $sitePackages)
    $pipExe = Join-Path $PythonDir "Scripts\pip.exe"
    if (-not (Test-Path $pipExe)) {
        $py = Join-Path $PythonDir "python.exe"
        $exit = Invoke-PythonWithOutput -PythonExe $py `
                    -Arguments "-m pip install --no-index --find-links `"$sitePackages`" pip" `
                    -StatusText "Finalizing pip..." -Progress 88
        if ($exit -ne 0) {
            $errDetail = Get-LastProcError
            Show-Error "pip wheel install failed`n`n$errDetail"
            return $false
        }
    }
    return $true
...
function Install-PythonPackages {
    ...
    if ($UseGetPip) {
        ...
        if ($exit -ne 0) {
            $errDetail = Get-LastProcError
            Update-GUI -Status "get-pip.py failed, trying pip wheel..." -Progress 87 -Detail $errDetail
            $ok = Install-PipFromWhl -PythonDir $PythonDir -DownloadDir $DownloadDir
    ...
    if ($RequirementsFile -ne "" -and (Test-Path $RequirementsFile)) {
        ...
        if ($exit -ne 0) {
            $errDetail = Get-LastProcError
            Show-Error "requirements.txt install failed (exit $exit)`n`n$errDetail"
            return $false
        }
    } else {
...
$form.Add_Shown({
    ...
    try {
        ...
        $getPipOk  = Download-File -Url $getPipUrl -DestinationPath $pipDest `
                                   -Description "get-pip.py" `
                                   -ProgressFrom 75 -ProgressTo 82
        $useGetPip = $getPipOk -and (Test-Path $pipDest)

        if ($useGetPip) {
            Rename-PthFiles -PythonPath $pythonDir
        } else {
            Update-GUI -Status "get-pip.py unavailable, will use pip wheel..." -Progress 82 -Detail ""
        }
        ...
        Update-GUI -Status "COMPLETE!" -Progress 100 `
                   -Detail "Python : $pythonDir`nVersion: $selVer ($selArch)`nPackages: $rep"
