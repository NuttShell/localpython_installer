# Get-PythonEmbedZips.ps1
# Finds, downloads and extracts python-X.Y.Z-embed-(amd64|win32).zip + get-pip.py
# Installs pip, pyinstaller, fonttools automatically
# Version constraints: X=1..4, Y=0..20, Z=0..10

function Test-InternetConnection {
    param(
        [string]$TestUrl = "https://www.python.org",
        [int]$TimeoutSec = 10
    )
    try {
        $response = Invoke-WebRequest -Uri $TestUrl -Method Head -UseBasicParsing -TimeoutSec $TimeoutSec -ErrorAction Stop
        return $response.StatusCode -eq 200
    }
    catch {
        try {
            $response = Invoke-WebRequest -Uri $TestUrl -UseBasicParsing -TimeoutSec $TimeoutSec -MaximumRedirection 2 -ErrorAction Stop
            return $response.StatusCode -eq 200
        }
        catch { return $false }
    }
...
function Get-UserSelection {
    ...
    while ($true) {
        ...
        if ($input -match '^\d+$' -and [int]$input -ge 0 -and [int]$input -le $Options.Count) {
            return [int]$input
        }
...
function Test-PythonVersion {
    ...
    if ($Version -match '^(\d+)\.(\d+)\.(\d+)$') {
        ...
        return ($major -ge 1 -and $major -le 4) -and 
               ($minor -ge 0 -and $minor -le 20) -and 
               ($patch -ge 0 -and $patch -le 10)
    }
    return $false
}
...
function Extract-PythonZip {
    ...
    try {
        ...
        if (Test-Path $ExtractPath) {
            Write-Host "  Removing existing folder: $ExtractPath" -ForegroundColor Gray
        ...
        if (Test-Path $pythonExe) {
            ...
            Write-Host "WARNING: python.exe not found after extraction" -ForegroundColor Yellow
            return $false
        }
    }
    catch {
...
function Rename-PthFiles {
    ...
    if ($pthFiles.Count -eq 0) {
        Write-Host "  No *._pth files found" -ForegroundColor Gray
        return $true
    }
    ...
    foreach ($file in $pthFiles) {
        ...
        if (-not (Test-Path $backupPath)) {
            ...
            Write-Host "  Skipped (backup exists): $($file.Name)" -ForegroundColor Gray
        }
    }
...
# 🔹 ФУНКЦИЯ: Настройка PATH и установка пакетов
function Install-PythonPackages {
    ...
    # 🔹 Добавляем Python и Scripts в PATH (для текущей сессии)
    ...
    if (Test-Path $getPipPath) {
        ...
        if ($proc.ExitCode -eq 0) {
            ...
            Write-Host "  WARNING: pip installation returned exit code $($proc.ExitCode)" -ForegroundColor Yellow
        }
    }
    else {
    ...
    if ($proc.ExitCode -eq 0) {
        ...
        Write-Host "  WARNING: pip upgrade returned exit code $($proc.ExitCode)" -ForegroundColor Yellow
    }
    
    # 🔹 Установка pyinstaller и fonttools
    ...
    if ($proc.ExitCode -eq 0) {
        Write-Host "  pyinstaller and fonttools installed successfully" -ForegroundColor Green
    }
    else {
        Write-Host "  WARNING: package installation returned exit code $($proc.ExitCode)" -ForegroundColor Yellow
    }
...
if (Test-Path $downloadDir) {
    ...
    try {
        ...
        Write-Host "Download folder removed successfully" -ForegroundColor Gray
    }
    catch {
        Write-Host "WARNING: Could not remove $downloadDir : $($_.Exception.Message)" -ForegroundColor Yellow
        exit 1
    }
}
...
if (Test-Path $pythonDir) {
    ...
    try {
        ...
        Write-Host "Python folder removed successfully" -ForegroundColor Gray
    }
    catch {
        Write-Host "WARNING: Could not remove $pythonDir : $($_.Exception.Message)" -ForegroundColor Yellow
        exit 1
    }
}
...
try {
    ...
    $versionFolders = $mainPage.Links | 
        Where-Object { $_.href -and $_.href -match $versionFolderPattern } | 
    ...
    foreach ($folder in $versionFolders) {
        ...
        try {
            ...
            $matches = $folderPage.Links | 
                Where-Object { $_.href -and $_.href -match $embedZipPattern } | 
                Select-Object -ExpandProperty href -Unique
    ...
    if ($allFiles.Count -eq 0) {
        Write-Host "No embed zip files found matching version filter." -ForegroundColor Yellow
        exit 0
    }
    ...
    if (-not $selectedFile) {
        Write-Host "ERROR: File not found for selected options." -ForegroundColor Red
        exit 1
    }
    ...
    if ($result -and (Test-Path $destinationPath)) {
        ...
        Write-Host "`n[FAILED] Python embed ZIP download failed." -ForegroundColor Red
        exit 1
    }
    ...
    if (-not $extractResult) {
        Write-Host "WARNING: Extraction failed or incomplete" -ForegroundColor Yellow
    }
    
    # Переименование *._pth файлов
    ...
    if ($getPipResult -and (Test-Path $getPipPath)) {
        ...
        Write-Host "[WARNING] get-pip.py download failed" -ForegroundColor Yellow
    }
    ...
    Write-Host "  - get-pip.py:    ✓" -ForegroundColor Gray
